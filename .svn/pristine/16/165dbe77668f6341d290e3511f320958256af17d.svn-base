...
public class SlideoutHelper {
	...
	/**
	 * capture a image of current view,which is placed on the right
	 * of the menu.Use the frameLayout and imageLayout to construct 
	 * a facebook-like style
	 * @param activity
	 * @param id
	 * @param width
	 */
	public static void prepare(Activity activity, int id, int width) {
		if (null != sCoverBitmap) {
			sCoverBitmap.recycle();
		}
		Rect rectgle = new Rect();
		Window window = activity.getWindow();
		window.getDecorView().getWindowVisibleDisplayFrame(rectgle);
		int statusBarHeight = rectgle.top;// the height of status bar

		ViewGroup v1 = (ViewGroup) activity.findViewById(id).getRootView();
		v1.setDrawingCacheEnabled(true);
		Bitmap source = Bitmap.createBitmap(v1.getDrawingCache());
		v1.setDrawingCacheEnabled(false);//clear cache,or it will return last cached pic when trying to perform this again
	...
	/**
	 * 
	 * @param activity  menu activity
 	 * @param reverse the position of the menu,false -->left
	 */
	...
	/**
	 * initialize the activity and animations
	 * 	 
	 * */
	public void activate() {
		...
		if (mReverse) {
			...
			final android.widget.AbsoluteLayout.LayoutParams lp = new android.widget.AbsoluteLayout.LayoutParams(
					android.view.ViewGroup.LayoutParams.FILL_PARENT, android.view.ViewGroup.LayoutParams.FILL_PARENT, x, 0);
			mActivity.findViewById(R.id.slideout_placeholder).setLayoutParams(
					lp);
		} else {
			@SuppressWarnings("deprecation")
			final android.widget.AbsoluteLayout.LayoutParams lp = new android.widget.AbsoluteLayout.LayoutParams(
					android.view.ViewGroup.LayoutParams.FILL_PARENT, android.view.ViewGroup.LayoutParams.FILL_PARENT, 0, 0);
			mActivity.findViewById(R.id.slideout_placeholder).setLayoutParams(
					lp);
		}
		initAnimations();
...
//		Toast.makeText(mActivity, "shift is "+shift, Toast.LENGTH_SHORT).show();
		mStartAnimation = new TranslateAnimation(Animation.ABSOLUTE,
				0, Animation.ABSOLUTE, -shift,
				Animation.ABSOLUTE, 0, Animation.ABSOLUTE, 0);

		mStopAnimation = new TranslateAnimation(Animation.ABSOLUTE, 0,
